...
const SavedPage = ({ currentLanguage }: SavedPageProps) => {
  ...
  const [recommendations, setRecommendations] = useState<SavedItem[]>([]);
  ...
  useEffect(() => {
    ...
    const mockRecommendations: SavedItem[] = [
      {
        id: "4",
        name: "Railay Beach",
  ...
  const renderItemCard = (item: SavedItem) => {
    if (viewMode === "grid") {
      return (
        <div key={item.id} className="bg-card rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border border-border/50 relative">
          <div className="relative overflow-hidden rounded-t-2xl">
            ...
            <div className="absolute top-3 right-3 flex space-x-2 z-20">
              <Button
                variant="ghost"
                size="icon"
                className="bg-white/90 hover:bg-destructive hover:text-destructive-foreground"
                onClick={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        {(favorites.length > 0 || savedItems.length > 0) &&
