...
export const getVideos = async (page: number = 1, limit: number = 10) => {
  const { data } = await apiClient.get(`/explore/videos?page=${page}&limit=${limit}`);
  return data;
};
...
export const getComments = async (videoId: string, page: number = 1) => {
  const { data } = await apiClient.get(`/videos/${videoId}/comments?page=${page}`);
  return data;
};
...
export const postComment = async (videoId: string, text: string) => {
  ...
  const { data } = await apiClient.post(`/videos/${videoId}/comments`, { input: text });
  return data;
};
