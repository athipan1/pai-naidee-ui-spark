    ...
    <div className="p-6 bg-white dark:bg-gray-800 rounded-lg shadow-lg max-w-md mx-auto">
      ...
      <div className="mb-6">
        ...
        <div className="space-y-2">
          ...
          <div className={`flex items-center gap-2 ${isTablet ? 'text-green-600' : 'text-gray-400'}`}>
            <span className={`w-3 h-3 rounded-full ${isTablet ? 'bg-green-500' : 'bg-gray-300'}`}></span>
            Tablet: {isTablet ? 'Yes' : 'No'}
          </div>
          <div className={`flex items-center gap-2 ${isDesktop ? 'text-green-600' : 'text-gray-400'}`}>
            <span className={`w-3 h-3 rounded-full ${isDesktop ? 'bg-green-500' : 'bg-gray-300'}`}></span>
      ...
      <div>
        ...
        <div className="space-y-2 text-sm">
          <div className={`flex items-center gap-2 ${isSmallScreen ? 'text-orange-600' : 'text-gray-400'}`}>
            <span className={`w-2 h-2 rounded-full ${isSmallScreen ? 'bg-orange-500' : 'bg-gray-300'}`}></span>
            Small Screen (≤480px): {isSmallScreen ? 'Yes' : 'No'}
          </div>
          <div className={`flex items-center gap-2 ${isLargeScreen ? 'text-purple-600' : 'text-gray-400'}`}>
            <span className={`w-2 h-2 rounded-full ${isLargeScreen ? 'bg-purple-500' : 'bg-gray-300'}`}></span>
            Large Screen (≥1440px): {isLargeScreen ? 'Yes' : 'No'}
          </div>
          <div className={`flex items-center gap-2 ${prefersDarkMode ? 'text-indigo-600' : 'text-gray-400'}`}>
            <span className={`w-2 h-2 rounded-full ${prefersDarkMode ? 'bg-indigo-500' : 'bg-gray-300'}`}></span>
            Prefers Dark Mode: {prefersDarkMode ? 'Yes' : 'No'}
          </div>
          <div className={`flex items-center gap-2 ${hasHover ? 'text-teal-600' : 'text-gray-400'}`}>
            <span className={`w-2 h-2 rounded-full ${hasHover ? 'bg-teal-500' : 'bg-gray-300'}`}></span>
            Has Hover: {hasHover ? 'Yes' : 'No'}
          </div>
          <div className={`flex items-center gap-2 ${isHighDPI ? 'text-pink-600' : 'text-gray-400'}`}>
            <span className={`w-2 h-2 rounded-full ${isHighDPI ? 'bg-pink-500' : 'bg-gray-300'}`}></span>
