...
const AttractionCard = ({
  ...
  return (
    <div className="attraction-card group cursor-pointer" onClick={() => onCardClick(id)}>
      {/* Image Container */}
      <div className="relative overflow-hidden rounded-t-2xl">
        <div className={`w-full h-48 bg-muted animate-pulse ${imageLoaded ? 'hidden' : 'block'}`} />
        <img 
          src={image} 
          alt={displayName}
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-1">
              ...
              <span className="text-xs text-muted-foreground">
                ({reviewCount}{currentLanguage === 'th' ? ' รีวิว' : ' reviews'})
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-1">
          ...
          {tags.length > 3 && (
            <span className="px-2 py-1 text-xs text-muted-foreground">
              +{tags.length - 3} {currentLanguage === 'th' ? 'เพิ่มเติม' : 'more'}
            </span>
          )}
        </div>
      </div>
